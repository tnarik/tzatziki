...
module Tzatziki
  class Mappings
    ...
    def initialize(features = nil, runtime = nil)
    ...
    def test_step(step, mapper)
      ...
      if step.multiline_arg.kind_of? Cucumber::Core::Ast::EmptyMultilineArgument
        multiline_arg = nil
      else
        multiline_arg = Cucumber::Core::Ast::MultilineArgument.from(step.multiline_arg, step.location) 
      end
    ...
    def runtime
      ...
      support_files.each do |support_file|
        p "loading #{support_file}"
        load File.expand_path(support_file)
      end
      @runtime = result
    end
